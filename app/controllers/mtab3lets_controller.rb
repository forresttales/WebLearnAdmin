class Mtab3letsController < ApplicationController
...
#       
        # @mtab3lets = Mtab3let.where(:id => '70')
#           
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @mtab3lets = Mtab3let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@mtab3lets = @mtab3lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @mtab3lets = Mtab3let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@mtab3lets = Mtab3let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @mtab3lets = Mtab3let.where(:id => '99')
      ...
      @mtab3lets = Mtab3let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @mtab3lets = Mtab3let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/mtab3let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO mtab3lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def show
    @mtab3let = Mtab3let.find(params[:id])    
  end
  ...
  def edit
    @mtab3let = Mtab3let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @mtab3let.update_attributes(params[:mtab3let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @mtab3let.id)
  ...
  def destroy
    ...
    if mtab3let.delete
      @@delete = true
      redirect_to(mtab3lets_index_path)
    else
  ...
  private
    ...
    def mtab3lets_params
      params.require(:mtab3let).permit(
                                    :col_1,
                                    :col_2,
                                    :col_3,
                                    :col_4,
    ...
    def sort_column
      Mtab3let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

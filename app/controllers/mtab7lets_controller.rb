class Mtab7letsController < ApplicationController
  ...
  def import
    ...
    CSV.foreach(Rails.root.join(file_dir), {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO mtab7lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  private
    ...
    def mtab7lets_params
      params.require(:mtab7let).permit(
                                    :col_1,
                                    :col_2,
                                    :col_3,
                                    :col_4,
    ...
    def sort_column
      Mtab7let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

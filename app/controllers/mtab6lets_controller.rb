class Mtab6letsController < ApplicationController
...
#       
        # @mtab6lets = Mtab6let.where(:id => '70')
#           
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @mtab6lets = Mtab6let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@mtab6lets = @mtab6lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @mtab6lets = Mtab6let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@mtab6lets = Mtab6let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @mtab6lets = Mtab6let.where(:id => '99')
      ...
      @mtab6lets = Mtab6let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @mtab6lets = Mtab6let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/mtab6let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO mtab6lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def show
    @mtab6let = Mtab6let.find(params[:id])    
  end
  ...
  def edit
    @mtab6let = Mtab6let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @mtab6let.update_attributes(params[:mtab6let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @mtab6let.id)
  ...
  def destroy
    ...
    if mtab6let.delete
      @@delete = true
      redirect_to(mtab6lets_index_path)
    else
  ...
  private
    ...
    def mtab6lets_params
      params.require(:mtab6let).permit(
                                    :col_1,
                                    :col_2,
                                    :col_3,
                                    :col_4,
    ...
    def sort_column
      Mtab6let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

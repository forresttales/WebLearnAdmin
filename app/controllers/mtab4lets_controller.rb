class Mtab4letsController < ApplicationController
...
#       
        # @mtab4lets = Mtab4let.where(:id => '70')
#           
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @mtab4lets = Mtab4let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@mtab4lets = @mtab4lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @mtab4lets = Mtab4let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@mtab4lets = Mtab4let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @mtab4lets = Mtab4let.where(:id => '99')
      ...
      @mtab4lets = Mtab4let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @mtab4lets = Mtab4let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/mtab4let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO mtab4lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def show
    @mtab4let = Mtab4let.find(params[:id])    
  end
  ...
  def edit
    @mtab4let = Mtab4let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @mtab4let.update_attributes(params[:mtab4let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @mtab4let.id)
  ...
  def destroy
    ...
    if mtab4let.delete
      @@delete = true
      redirect_to(mtab4lets_index_path)
    else
  ...
  private
    ...
    def mtab4lets_params
      params.require(:mtab4let).permit(
                                    :col_1,
                                    :col_2,
                                    :col_3,
                                    :col_4,
    ...
    def sort_column
      Mtab4let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

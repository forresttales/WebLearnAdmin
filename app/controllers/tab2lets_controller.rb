class Tab2letsController < ApplicationController
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @tab2lets = Tab2let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@tab2lets = @tab2lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @tab2lets = Tab2let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@tab2lets = Tab2let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @tab2lets = Tab2let.where(:id => '99')
      ...
      @tab2lets = Tab2let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @tab2lets = Tab2let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/tab2let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO tab2lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def upload
    ...
    # @tab2lets = Tab2let.order(:id)
    # respond_to do |format|
      # format.html
      # format.csv { send_data @tab2lets.to_csv }
...
# 
  ...
  def show
    @tab2let = Tab2let.find(params[:id])    
  end
  ...
  def edit
    @tab2let = Tab2let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @tab2let.update_attributes(params[:tab2let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @tab2let.id)
  ...
  def destroy
    ...
    if tab2let.delete
      @@delete = true
      redirect_to(tab2lets_index_path)
    else
  ...
  private
    ...
    def sort_column
      Tab2let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

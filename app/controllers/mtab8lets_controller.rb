...
#       
        # @mtab8lets = Mtab8let.where(:id => '70')
#           
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      ...
      #@mtab8lets = Mtab8let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @mtab8lets = Mtab8let.where(:id => '99')
  ...
  def export
    ...
    # @mtab8lets = Mtab8let.where(params[:export])
  ...
  def update
    ...
    if @mtab8let.update_attributes(params[:mtab8let])
      ...
      redirect_to(:action => 'show', :saved_id => @mtab8let.id)
  ...
  def destroy
    ...
    if mtab8let.delete
      @@delete = true
      redirect_to(mtab8lets_index_path)
    else

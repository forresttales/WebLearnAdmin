class Mtab5letsController < ApplicationController
...
#       
        # @mtab5lets = Mtab5let.where(:id => '70')
#           
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @mtab5lets = Mtab5let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@mtab5lets = @mtab5lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @mtab5lets = Mtab5let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@mtab5lets = Mtab5let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @mtab5lets = Mtab5let.where(:id => '99')
      ...
      @mtab5lets = Mtab5let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @mtab5lets = Mtab5let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/mtab5let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO mtab5lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def show
    @mtab5let = Mtab5let.find(params[:id])    
  end
  ...
  def edit
    @mtab5let = Mtab5let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @mtab5let.update_attributes(params[:mtab5let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @mtab5let.id)
  ...
  def destroy
    ...
    if mtab5let.delete
      @@delete = true
      redirect_to(mtab5lets_index_path)
    else
  ...
  private
    ...
    def mtab5lets_params
      params.require(:mtab5let).permit(
                                    :col_1,
                                    :col_2,
                                    :col_3,
                                    :col_4,
    ...
    def sort_column
      Mtab5let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end

class Tab1letsController < ApplicationController
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @tab1lets = Tab1let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@tab1lets = @tab1lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @tab1lets = Tab1let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@tab1lets = Tab1let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @tab1lets = Tab1let.where(:id => '99')
      ...
      @tab1lets = Tab1let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @tab1lets = Tab1let.where(params[:export])
  ...
  def import
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO tab1lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def upload
    ...
    # @tab1lets = Tab1let.order(:id)
    # respond_to do |format|
      # format.html
      # format.csv { send_data @tab1lets.to_csv }
...
# 
  ...
  def show
    @tab1let = Tab1let.find(params[:id])    
  end
  ...
  def edit
    @tab1let = Tab1let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @tab1let.update_attributes(params[:tab1let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @tab1let.id)
  ...
  def destroy
    ...
    if tab1let.delete
      @@delete = true
      redirect_to(tab1lets_index_path)
    else
  ...
  private
    ...
    def sort_column
      Tab1let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end
